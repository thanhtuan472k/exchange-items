...
export default function Chat() {
    ...
    useEffect(() => {
        if (param.id) {
            ...
            const isFriend = conversations.find((conversation) => conversation.members.includes(param.id));
    ...
    const handleChatRoom = (chatRoom) => {
        ...
        const sellerInfo = chatRoom.membersInfo.find((member) => member._id !== user._id);
    ...
    const handleSubmit = async (e) => {
        ...
        const receiverId = currentChat.membersInfo.find((member) => member._id !== user._id);
                        ...
                        {chatRooms.map((chatRoom) => {
                            return (
                                <div onClick={() => handleChatRoom(chatRoom)}>
                                    <Conversations key={chatRoom._id} chatRoom={chatRoom} currentUser={user} />
                                </div>
                            );
                        })}
                    </div>
